...
2. **开启卡密**：在文章编辑页的 **TypechoPay 付费功能** 面板中选择“卡密管理”，填写价格、购买权限、购买策略和库存显示。保存时插件会自动创建或更新 `content_id = 当前文章 cid` 的商品，并可自动在正文开头插入 `[typechopay_product]`。
3. **导入卡密**：保存文章后，回到文章编辑页可通过“查看库存/查看销售”进入运营页；也可以到 TypechoPay → 商品管理 → 导入卡密。支持 `.txt/.csv/.tsv` 文件或粘贴文本，预览确认后才正式导入。
4. **编辑商品**：可继续在文章编辑页修改付费设置，也可在商品管理页修改标题、价格、状态、购买策略、登录购买规则和交付规则；这些影响购买或交付的规则变化会自动递增商品版本号。
