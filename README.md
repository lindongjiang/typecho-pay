...
- **支付宝配置**：支付模式、AppID、网关地址、应用私钥、支付宝公钥、Seller ID。普通公钥模式的应用私钥和支付宝公钥直接粘贴到插件设置中保存，不需要单独上传密钥文件；升级或停用再启用时会尽量从配置备份恢复显示。
...
- 当前只支持支付宝普通公钥模式，暂不支持公钥证书模式。插件设置页直接保存应用私钥和支付宝公钥文本，并会把只粘贴正文的密钥规范化为 PEM。
...
- 卡密正文使用 Typecho 站点密钥派生出的 AES-256-GCM 密钥加密保存；后台文章底部、卡密库存和卡密销售会为管理员解密显示完整卡密，便于运营核对。不要随意更换站点密钥，否则历史卡密无法解密；后台截图也不要外泄。
