...
1. **创建商品**：TypechoPay → 商品管理 → 创建商品，勾选”交付卡密”，可设置 `max_per_user`。
2. **导入卡密**：选择商品 → 粘贴卡密或上传 `.txt/.csv/.tsv` 文件。支持 `卡号----卡密`、`卡号|卡密`、`卡号,卡密`、Tab 分隔或单独兑换码。导入前会预查已有指纹，报告原始行数、文件内重复、数据库重复和成功数。
3. **编辑商品**：点击商品列表中的”编辑”可修改标题、价格、状态、购买策略、交付规则；价格或策略变更时自动递增版本号。
4. **查看库存**：TypechoPay → 卡密库存 → 按商品/状态/批次筛选，支持标记作废和标记泄露。
5. **查看销售**：TypechoPay → 卡密销售 → 已交付卡密关联订单号、金额、网关、买家、交付状态、补发次数和最后错误。
6. 在文章中使用短代码：
...
7. 用户点击支付时，系统先从 `available` 库存中条件更新预留一张卡密为 `reserved`，预留 30 分钟。同一买家、同一商品、同一网关的未过期活动订单会被复用，不会重复创建支付平台订单。
8. 支付成功后，`FulfillmentManager` 把同一张预留卡密标记为 `delivered`。即使卡密交付暂时失败，支付确认和 ACK 仍会成功，不会导致支付平台重复回调。
9. 用户支付页轮询到成功后会跳转到卡密交付页。访问通过 HttpOnly delivery cookie、订单所有者校验完成。
10. 过期预留释放时会跳过已支付订单的卡密，防止已付款订单丢失预留。
